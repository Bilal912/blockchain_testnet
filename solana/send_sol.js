...
// Configuration
const PRIVATE_KEY_STRING = '2RjLGpnn7kcDDrSkjvgEkyvjKxZ2SGjeZKbZ7n3SbPz1DM2rYzqo6S9kzTAjhMiyFJKPYju4UGJ6i7rW3isHKCzh';
...
async function sendSolanaTransaction() {
  try {
    ...
    const recipientPublicKey = new solanaWeb3.PublicKey(RECIPIENT_ADDRESS_STRING);

    // Get sender's balance
    const senderBalanceLamports = await connection.getBalance(senderKeypair.publicKey);
    const senderBalanceSOL = senderBalanceLamports / solanaWeb3.LAMPORTS_PER_SOL;
    ...
    // Send transaction
    console.log(`Sending ${AMOUNT_TO_SEND_SOL} SOL from ${senderKeypair.publicKey.toBase58()} to ${RECIPIENT_ADDRESS_STRING}...`);
